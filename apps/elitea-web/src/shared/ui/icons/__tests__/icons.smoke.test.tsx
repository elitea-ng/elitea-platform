...
describe('shared/ui/icons — full-set smoke test', () => {
  it('discovered the full ported set (201 icons, see final report for the merge/rename ledger)', () => {
    ...
    // theme toggle's "System") + two the settings-parity pass added:
    // `CompassIcon` (Settings › Project Context; the briefcase it used to
    // borrow now belongs to Settings › General) and `ContextIcon` (the glyph
    // above Project Context's empty state). `SortArrowsIcon`, which the
    // Notifications table's sortable headers now draw, was already here and
    // had no call site.
    expect(icons.length).toBe(201);
  });
