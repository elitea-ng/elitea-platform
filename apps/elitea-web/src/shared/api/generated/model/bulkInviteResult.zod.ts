...
export const BulkInviteResult = zod
  .object({
    user_id: zod.int(),
    user_email: zod
      .string()
      .describe("Empty when the id resolved to no account."),
    project_id: zod.int(),
    project_name: zod
      .string()
      .describe("Empty when the id resolved to no project."),
    status: zod
      .enum(["ok", "error"])
      .describe(
        "The coarse verdict. `ok` covers both `added` and `already_member`: the account holds the role in the project either way.\n",
      ),
    outcome: zod
      .enum([
        "added",
...
export type BulkInviteResultOutput = zod.output<typeof BulkInviteResult>;
