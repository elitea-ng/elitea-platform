...
function parseChunk(value: unknown, maximum: number): ToolOutputChunk | undefined {
  if (!value || typeof value !== 'object') return undefined;
  const row = value as Record<string, unknown>;
  if (typeof row['offset_bytes'] !== 'number' || !Number.isSafeInteger(row['offset_bytes']) || row['offset_bytes'] < 0 ||
      typeof row['total_bytes'] !== 'number' || !Number.isSafeInteger(row['total_bytes']) || row['total_bytes'] <= 0 || row['total_bytes'] > maximum ||
...
export function appendToolOutputChunk(previous: unknown, output: unknown, metadata: unknown, priorMetadata: unknown, maximum = 1048576): { output: string; chunk: ToolOutputChunk } | undefined {
  const chunk = parseChunk(metadata, maximum);
  ...
  const prior = parseChunk(priorMetadata, maximum);
