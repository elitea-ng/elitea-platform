...
func TestMainWiring(t *testing.T) {
	...
	required := []struct {
		...
		{"budgetStartupGate(", "the startup refusal is never computed — a gateway whose GATEWAY_NATS_URL is empty while the database holds enforcing budget rows would start, serve /llm with every authored ceiling ignored, bill nothing, and report ready, because nothing re-dials a URL that was never given and the /readyz gate is scoped to a CONFIGURED NATS (issue #304)"},
