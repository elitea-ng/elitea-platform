...
| 3c Toolkit Test | `toolkit-test-reference-binding.md`, `toolkit-cancellation-acceptance.md`, `toolkit-test-result-recovery.md`: invocation, authorization references, administrative cancellation, result/reload observation. | `mcp-toolkit-capability-20260914.md`: MCP admission is fixed; discovery still loses its authorization challenge, blocking browser OAuth verification. Loss before the client receives an execution ID remains open. |
