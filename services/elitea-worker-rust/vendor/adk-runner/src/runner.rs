...
impl Runner {
    /// Refresh completed-event history from the durable session view.
    ///
    /// The service must return a complete, identity-scoped model view.
    /// Partial chunks remain streamed but are not accumulated as history.
    /// Disabled by default to preserve existing session-service behavior.
    pub fn with_session_event_refresh(mut self, enabled: bool) -> Self {
        self.refresh_session_events = enabled;
        self
    }

    async fn refresh_events_from_storage(
        service: &dyn SessionService,
        identity: &adk_core::AdkIdentity,
        session: &crate::context::MutableSession,
    ) -> Result<()> {
        let current = service.get(adk_session::GetRequest {
        ...
        session.replace_events(current.events().all());
        Ok(())
    }
    ...
    pub async fn run_with_config(
        ...
        let refresh_session_events = self.refresh_session_events;
            ...
            while let Some(result) = {
                ...
                match result {
                    Ok(mut event) => {
                        ...
                        if !refresh_session_events || !event.llm_response.partial {
                        ...
                        if refresh_session_events && !event.llm_response.partial {
                            if let Err(error) = Self::refresh_events_from_storage(
                                session_service.as_ref(), &identity, ctx.mutable_session(),
                            ).await {
                                #[cfg(feature = "plugins")]
                                if let Some(manager) = plugin_manager.as_ref() {
                                    manager.run_after_run(ctx.clone() as Arc<dyn adk_core::InvocationContext>).await;
                ...
                while let Some(result) = {
                    ...
                    match result {
                        Ok(mut event) => {
                            ...
                            if refresh_session_events && !event.llm_response.partial {
                                if let Err(error) = Self::refresh_events_from_storage(
                                    session_service.as_ref(), &identity, transfer_ctx.mutable_session(),
                                ).await {
                                    #[cfg(feature = "plugins")]
                                    if let Some(manager) = plugin_manager.as_ref() {
                                        manager.run_after_run(ctx.clone() as Arc<dyn adk_core::InvocationContext>).await;
