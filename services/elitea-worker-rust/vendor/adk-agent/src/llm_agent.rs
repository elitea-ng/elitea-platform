...
impl LlmAgentBuilder {
    ...
    /// Retain callback-prepared history as the input for the next model cycle.
    ///
    /// Opt in only when callbacks own complete history preparation. The default
    /// keeps callback changes local to one request for backward compatibility.
    pub fn retain_prepared_history(mut self, enabled: bool) -> Self {
        self.retain_prepared_history = enabled;
        self
    }
    ...
    pub fn build(self) -> Result<LlmAgent> {
        ...
        Ok(LlmAgent {
            ...
            retain_prepared_history: self.retain_prepared_history,
...
impl Agent for LlmAgent {
    ...
    async fn run(&self, ctx: Arc<dyn InvocationContext>) -> Result<adk_core::EventStream> {
        ...
        let retain_prepared_history = self.retain_prepared_history;
        ...
        let s = stream! {
            ...
            loop {
                ...
                if retain_prepared_history && model_response_override.is_none() {
                    conversation_history = request.contents.clone();
                }
