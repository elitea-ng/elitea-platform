...
const discoveryRequestDigestDomain = "elitea.toolkit.available_tools.admission.v1\x00"

var ErrInvalidDiscoveryAdmission = errors.New("invalid toolkit discovery admission")
...
type AtomicAdmissionStore interface {
	AdmitToolkitAvailableTools(context.Context, Admission) (executionapp.AdmissionOutcome, error)
}
...
func NewAdmissionService(
	...
	if store == nil || factory == nil || newID == nil {
		return nil, errors.New("toolkit discovery admission dependencies are required")
	}
	if now == nil {
		now = time.Now
	}
...
func (s *AdmissionService) Submit(ctx context.Context, request SubmitRequest) (AdmittedRun, error) {
	...
	if !validDiscoveryIdentity(request.Identity) ||
		request.IdempotencyKey == "" || len(request.IdempotencyKey) > 200 {
		return AdmittedRun{}, ErrInvalidDiscoveryAdmission
	}
	bundle, binding, err := s.factory.Build(ctx, request.Inputs.Clone())
	if err != nil {
		return AdmittedRun{}, fmt.Errorf("%w: %v", ErrInvalidDiscoveryAdmission, err)
	}
	requestDigest := discoveryRequestDigest(request, binding, bundle)
	...
	outcome, err := s.store.AdmitToolkitAvailableTools(ctx, Admission{
		Record:  record,
		Binding: binding,
	})
	if err != nil {
		return AdmittedRun{}, fmt.Errorf("admit toolkit discovery: %w", err)
	}
	if outcome.ExecutionID == "" || outcome.CommandID == "" ||
		outcome.AdmittedAt.IsZero() || !outcome.Deadline.After(outcome.AdmittedAt) {
		return AdmittedRun{}, errors.New("toolkit discovery admission store returned invalid durable outcome")
	}
	return AdmittedRun{
		Outcome:     outcome,
		Binding:     binding,
...
func (s *AdmissionService) allocateAdmissionIDs() (string, string, string, error) {
	...
	for index := range values {
		value, err := s.newID()
		if err != nil {
			return "", "", "", fmt.Errorf("generate toolkit discovery admission ID: %w", err)
		}
		if value == "" {
			return "", "", "", errors.New("toolkit discovery admission ID generator returned an empty ID")
		}
		values[index] = value
	}
...
func validDiscoveryIdentity(identity executionapp.AdmissionIdentity) bool {
	return identity.TenantID != "" && identity.ResourceProjectID != "" &&
		identity.ProjectionProjectID != "" && identity.ActorID != ""
}

func discoveryRequestDigest(
	...
	values := [][]byte{
		...
		[]byte(executiondomain.ToolkitAvailableToolsCapability),
	...
	material = append(material, discoveryRequestDigestDomain...)
